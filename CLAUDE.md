...
- **Reviews section** (`reviews/`) — clean-URL directories (`reviews/index.html` → `/reviews/`, `reviews/<slug>/index.html`). Review *state* (status, rating, affiliateUrl, specs.verified) lives in `assets/js/reviews-data.js` and is enforced by `assets/js/reviews.js`: a draft never links out in production, a null rating renders nothing, and a null affiliateUrl renders an inert CTA. Body copy lives in the page HTML for SEO. Read `REVIEWS.md` before adding or publishing a review. Note `.github/workflows/pages.yml` must keep `reviews/**` in its path filter or new review pages will not deploy.
